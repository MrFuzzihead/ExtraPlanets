package com.mjr.extraplanets.planets.Jupiter.worldgen.village;

import java.util.List;
import java.util.Random;

import net.minecraft.world.gen.structure.StructureBoundingBox;
import net.minecraft.world.gen.structure.StructureComponent;

import com.mjr.extraplanets.blocks.ExtraPlanets_Blocks;
import com.mjr.extraplanets.worldgen.village.StructureComponentVillageHouse;
import com.mjr.extraplanets.worldgen.village.StructureComponentVillageStartPiece;

public class StructureComponentJupiterVillageHouse extends StructureComponentVillageHouse {

    public StructureComponentJupiterVillageHouse() {
        this.planetBlock = ExtraPlanets_Blocks.jupiterBlocks;
    }

    public StructureComponentJupiterVillageHouse(StructureComponentVillageStartPiece par1ComponentVillageStartPiece,
        int par2, Random par3Random, StructureBoundingBox par4StructureBoundingBox, int par5) {
        super(par1ComponentVillageStartPiece, par2, par3Random, par4StructureBoundingBox, par5);
        this.planetBlock = ExtraPlanets_Blocks.jupiterBlocks;
    }

    @SuppressWarnings("rawtypes")
    public static StructureComponentJupiterVillageHouse func_74921_a(
        StructureComponentVillageStartPiece par0ComponentVillageStartPiece, List par1List, Random par2Random, int par3,
        int par4, int par5, int par6, int par7) {
        final StructureBoundingBox var8 = StructureBoundingBox
            .getComponentToAddBoundingBox(par3, par4, par5, 0, 0, 0, 17, 9, 17, par6);
        return StructureComponent.findIntersecting(par1List, var8) == null
            ? new StructureComponentJupiterVillageHouse(par0ComponentVillageStartPiece, par7, par2Random, var8, par6)
            : null;
    }
}
