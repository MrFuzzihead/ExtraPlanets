...
public class GuiPoweredVehicleBase extends GuiContainerGC {
    ...
    static {
        for (int i = 0; i < 4; i++) {
            GuiPoweredVehicleBase.sealerTexture[i] = new ResourceLocation(
                Constants.ASSET_PREFIX,
                "textures/gui/powered_vehicle_" + i * 18 + ".png");
        }
    }
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        if (this.mc.thePlayer != null && this.mc.thePlayer.ridingEntity != null
            && this.mc.thePlayer.ridingEntity instanceof EntityPoweredVehicleBase) {
            this.fontRendererObj
                .drawString(GCCoreUtil.translate("gui.message.power.message.name") + ":", 130, 15 + 3, 4210752);
            final int percentage = (int) (((EntityPoweredVehicleBase) this.mc.thePlayer.ridingEntity)
                .getCurrentPowerCapacity() / 100);
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        this.mc.getTextureManager()
            .bindTexture(GuiPoweredVehicleBase.sealerTexture[this.type]);
        ...
        if (this.mc.thePlayer != null && this.mc.thePlayer.ridingEntity != null
            && this.mc.thePlayer.ridingEntity instanceof EntityPoweredVehicleBase) {
            final int percentage = (int) ((int) (((EntityPoweredVehicleBase) this.mc.thePlayer.ridingEntity)
                .getCurrentPowerCapacity() / 100) / 2.6);
            this.drawTexturedModalRect(
                (this.width - this.xSize) / 2 + 95,
                (this.height - this.ySize) / 2 + 45 - percentage,
                176,
                38 - percentage,
                25,
                percentage);
