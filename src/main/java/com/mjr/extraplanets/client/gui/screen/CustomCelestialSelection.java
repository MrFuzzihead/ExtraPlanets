...
public class CustomCelestialSelection extends GuiCelestialSelection {
    ...
    public CustomCelestialSelection(MapMode mapMode, List<CelestialBody> possibleBodies) {
    ...
    public HashMap<CelestialBody, Matrix4f> drawCelestialBodies(Matrix4f worldMatrix) {
        ...
        for (SolarSystem solarSystem : GalaxyRegistry.getRegisteredSolarSystems()
            .values()) {
            ...
            if (star != null && star.getBodyIcon() != null) {
                ...
                if (alpha != 0) {
                    CelestialBodyRenderEvent.Pre preEvent = new CelestialBodyRenderEvent.Pre(
                        star,
                        star.getBodyIcon(),
                        8);
    ...
    public void drawButtons(int mousePosX, int mousePosY) {
        try {
            if (!(this.selectionState == EnumSelectionState.PROFILE)) {
                if (this.selectedBody != null) {
                    ...
                    for (int barY = 0; barY < 25; ++barY) {
                        this.drawTexturedModalRect(
                            xOffset,
                            yOffset + barY * this.fontRendererObj.FONT_HEIGHT + 4,
                            93 + widthSizeOffset,
                    ...
                    if (!(this.selectedBody instanceof Star)) {
                        ...
                        if (this.selectedBody.getReachable() && !this.selectedBody.getUnlocalizedName()
                            .contains("overworld") && !(this.selectedBody instanceof Satellite))
                            if (FMLCommonHandler.instance()
                                .getEffectiveSide() == Side.CLIENT)
                                temp = WorldUtil.getProviderForDimensionClient(this.selectedBody.getDimensionID());
                            else temp = WorldUtil.getProviderForDimensionServer(this.selectedBody.getDimensionID());
                        this.drawString(
                            this.fontRendererObj,
                            "------------------------",
                            xOffset + 10,
                            yOffset + 2,
                            ColorUtil.to32BitColor(255, 0, 150, 255));
                        this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.general_details.name") + ": ",
                            xOffset + 10,
                            yOffset + 8,
                            ColorUtil.to32BitColor(255, 0, 150, 255));
                        this.drawString(
                            ...
                            ColorUtil.to32BitColor(255, 0, 150, 255));
                        String type;
                        if (GalaxyRegistry.getRegisteredPlanets()
                            .containsValue(this.selectedBody)) type = "Planet";
                        else if (GalaxyRegistry.getRegisteredMoons()
                            .containsValue(this.selectedBody)) type = "Moon";
                        else if (GalaxyRegistry.getRegisteredSatellites()
                            .containsValue(this.selectedBody)) type = "Satellite";
                        else type = "Unknown";

                        this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.celestial_body_type.name") + ": " + type,
                            xOffset + 10,
                            yOffset + 23,
                            14737632);
                        if ((this.selectedBody instanceof Planet)) this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.celestial_body_amount_of_moons.name") + ": "
                                + this.getChildren(this.selectedBody)
                                    .size(),
                            xOffset + 10,
                            yOffset + 33,
                            14737632);
                        ...
                        this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.celestial_details.name") + ": ",
                            xOffset + 10,
                            yOffset + 8,
                            ColorUtil.to32BitColor(255, 0, 150, 255));
                        this.drawString(
                        ...
                        this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.celestial_body_orbit_time.name") + ": "
                                + this.selectedBody.getRelativeOrbitTime() * 24
                                + " days",
                            xOffset + 10,
                            yOffset + 23,
                            14737632);
                        float gravity = 0;
                        long dayLength = 0;
                        if (this.selectedBody.getReachable() && !(this.selectedBody instanceof Satellite)
                            && !this.selectedBody.getUnlocalizedName()
                                .toLowerCase()
                                .contains("overworld")) {
                            gravity = ((WorldProviderSpace) temp).getGravity();
                            dayLength = ((WorldProviderSpace) temp).getDayLength() / 1000;
                        } else if (this.selectedBody.getUnlocalizedName()
                            .toLowerCase()
                            .contains("overworld")) {
                                gravity = 1;
                                dayLength = 24;
                        ...
                        this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.celestial_body_gravity.name") + ": "
                                + (this.selectedBody.getReachable() ? gravity : "Unknown"),
                            xOffset + 10,
                            yOffset + 33,
                            14737632);
                        this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.celestial_body_day_lengh.name") + ": "
                                + (this.selectedBody.getReachable() ? dayLength + " hours" : "Unknown"),
                            xOffset + 10,
                            yOffset + 43,
                            14737632);
                        yOffset = yOffset + 55;
                        ...
                        this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.general_features_details.name") + ": ",
                            xOffset + 10,
                            yOffset + 8,
                            ColorUtil.to32BitColor(255, 0, 150, 255));
                        this.drawString(
                        ...
                        this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.celestial_body_meteor_frequency.name") + ": "
                                + (this.selectedBody.getReachable() ? meteorFrequency : "Unknown"),
                            xOffset + 10,
                            yOffset + 23,
                            14737632);

                        String name = this.selectedBody.getUnlocalizedName()
                            .toLowerCase();
                        String hasDungeon = "Unknown";
                        if (name.contains("moon") || name.contains("venus")
                            || name.contains("mars")
                            ...
                            || name.contains("pluto")
                            || name.contains("eris")) hasDungeon = "true";
                        if (name.contains("overworld") || name.contains("ceres")
                            || name.contains("kepler22b")
                            || name.contains("asteroids")
                            || name.contains("phobos")
                            || name.contains("deimos")
                            || name.contains("io")
                            || name.contains("europa")
                            || name.contains("ganymede")
                            || name.contains("callisto")
                            || name.contains("rhea")
                            || name.contains("titan")
                            || name.contains("iapetus")
                            || name.contains("titania")
                            || name.contains("oberon")
                            || name.contains("triton")
                            || name.contains("mercury")) hasDungeon = "false";
                        this.drawString(
                            this.fontRendererObj,
                            "Has Dungeon: " + hasDungeon,
                            xOffset + 10,
                            yOffset + 32,
                            14737632);
                        yOffset = yOffset + 5;
                        ...
                        this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.atmosphere_details.name") + ": ",
                            xOffset + 10,
                            yOffset + 45,
                            ColorUtil.to32BitColor(255, 0, 150, 255));
                        this.drawString(
                        ...
                        this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.celestial_body_wind_level.name") + ": "
                                + (this.selectedBody.getReachable() ? windLevel + "%" : "Unknown"),
                            xOffset + 10,
                            yOffset + 60,
                            14737632);
                        float temperature = 0;
                        if (this.selectedBody.getReachable() && !this.selectedBody.getUnlocalizedName()
                            .toLowerCase()
                            .contains("overworld") && !(this.selectedBody instanceof Satellite)) try {
                                temperature = ((WorldProviderSpace) temp).getThermalLevelModifier();
                            } catch (Exception e) {}
                        this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.celestial_body_temperature.name") + ": "
                                + (this.selectedBody.getReachable() ? temperature + "°C" : "Unknown"),
                            xOffset + 10,
                            yOffset + 70,
                            14737632);
                        ...
                        try {
                            ...
                            if (this.selectedBody.getUnlocalizedName()
                                .contains("overworld")) breathable = true;
                        } catch (Exception e) {}
                        this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.celestial_body_breathable.name") + ": "
                                + (this.selectedBody.getReachable() ? breathable : "Unknown"),
                            xOffset + 10,
                            yOffset + 80,
                            14737632);
                        ...
                        this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.celestial_body_type.name") + ": " + "Star",
                            xOffset + 10,
                            yOffset + 23,
                            14737632);
                        List<CelestialBody> planets = this
                            .getChildren(((Star) this.selectedBody).getParentSolarSystem());
                        int amountofPlanets = planets.size();
                        this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.celestial_body_amount_of_planets.name") + ": " + amountofPlanets,
                            xOffset + 10,
                            yOffset + 33,
                            14737632);
                        ...
                        this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.parent_solar_system_details.name") + ": ",
                            xOffset + 10,
                            yOffset + 8,
                            ColorUtil.to32BitColor(255, 0, 150, 255));
                        this.drawString(
                        ...
                        for (int i = 0; i < amountofPlanets; i++) {
                            amountofMoons += this.getChildren(planets.get(i))
                                .size();
                        }
                        this.drawString(
                            this.fontRendererObj,
                            GCCoreUtil.translate("gui.celestial_body_amount_of_moons.name") + ": " + amountofMoons,
                            xOffset + 10,
                            yOffset + 23,
                            14737632);
                    }
            ...
            System.out.println(
                "An error has occurred while rendering custom features on the Celestial Selection screen, Please check log for errors!");
            e.printStackTrace();
        }
